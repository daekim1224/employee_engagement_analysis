...
```
   - h. engagement factor scores for employees whose tenure are "10+ years".
    N = 2, given that this category has less than 5 employees, we don't report it.
```{r}
...
```
   - i. engagement factor scores for employees whose tenure are "hired on or after 2024-03-01".
     N = 11 (wait for what Vivian says)
```{r}
#hired on or after 2024-03-01 
df_2[df_2$tenure_group == "hired on or after 2024-03-01" & !is.na(df_2$tenure_group), ]

# Calculate favorability_score for columns 8 to 12 (engagement questions)
favorability_score <- function_favorability(df_2[df_2$tenure_group == "hired on or after 2024-03-01", 8:12])

colnames(favorability_score)[1] <- "question_number"
print(favorability_score)

# Calculate the average engagement factor favorable score for those who were hired on or after 2024-03-01
average_score <- mean(favorability_score$favorability_score, na.rm = TRUE)
...
ggplot(favorability_score_with_average_percent, aes(x = question_number, y = favorability_score, fill = highlight)) +
  geom_bar(stat = "identity", show.legend = FALSE, width = 0.6) +  
  labs(x = "Factor Items", y = "Favorability Score", title = "Engagement Favorability Scores for Employees Hired On Or After 2024-03-01") + 
  theme_minimal() + 
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1), 
    plot.title = element_text(hjust = 0.5)  
...
```
    ...
    - j. Engagement factor favorable scores across tenure groups (employee life cycle)
```{r}
