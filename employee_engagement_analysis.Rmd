...
# Remove 'All Employees' from the data frame
filtered_favorability_scores = combined_favorability_scores %>%
  filter(gender != "All Employees")

# Create a bar graph with ggplot2, only using data for 'Men' and 'Women'
ggplot(filtered_favorability_scores, aes(x = question_number, y = favorability_score, fill = gender)) +
  geom_bar(stat = "identity", position = "dodge") +  # Bars side-by-side
  labs(
    x = "Engagement Question",
    y = "Favorability Score (%)",
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 0, hjust = 1),  # Keep x-axis labels aligned
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    plot.caption = element_text(size = 6, hjust = 0)  # Style the caption
  ) +
  geom_text(aes(label = paste0(round(favorability_score, 1), "%")),  # Add percentage labels
            position = position_dodge(width = 0.9),
            vjust = -0.5,
            size = 3.5) +  # Adjust text size
  scale_fill_manual(  # Custom colors for gender
    values = c("Men" = "skyblue", "Women" = "purple")
  )
...
### Statistical Significant testing between men and women
<br>
<br>
```{r}
#two sample t-test
...
# Separate the data into subsets for men and women
men_engagement <- df_2 %>% filter(gender == "Male") %>% pull(engagement_factor)
women_engagement <- df_2 %>% filter(gender == "Female") %>% pull(engagement_factor)

# Conduct a two-sample t-test to compare the means of engagement factors between men and women
t_test_result <- t.test(men_engagement, women_engagement, var.equal = TRUE)  # Assumes equal variance

# Display the result of the t-test
print(t_test_result)

# Check if the difference is significant by examining the p-value
if (t_test_result$p.value < 0.05) {
  message("There is a statistically significant difference in engagement factor between men and women.")
} else {
  message("There is no statistically significant difference in engagement factor between men and women.")
}

#welch's t-test, relaxing equal variances between groups.
welch_test_result <- t.test(men_engagement, women_engagement, var.equal = FALSE)  

print(welch_test_result)

#Mann-Whitney U test (wilcoxon Rank-SUm test)- non-parametric test
mann_whitney_result <- wilcox.test(men_engagement, women_engagement)

print(mann_whitney_result)
#one way anova given that the DV is continuous and the IV is categorical
anova_result <- aov(engagement_factor ~ gender, data = df_2)  # One-way ANOVA with gender as factor
summary(anova_result)

#robust OLS (accounting for heteroscedasticity--unequal variances, outliers, relaxes non-normality)

# Load the MASS package for robust regression
library(MASS)

# Fit a robust regression model with the Huber method
robust_model <- rlm(engagement_factor ~ gender, data = df_2)
...
total_employees <- sum(country_df$Count)

country_df <- country_df %>%
  mutate(Proportion = Count / total_employees)  # Add a column for proportions
