---
title: "employee_engagement_analysis for Hooli"
...
```
  ...
  * 4. Correlate all non-engagement questions to the engagement factor for Hooli Overall.
      -  a. for all employees & all non-engagement items (Kendall's tau-b)
...
```
     d. for Indian employees (correlation calculated by conducting kendall's tau-b)
```{r}
library(dplyr)
...
```
    e. for German employees (correlation calculated by conducting kendall's tau-b)
```{r}
library(dplyr)
...
```
  f. for employees who are high (engagement factor composite score > or = 4) on employee_engagement (correlation calculated by conducting kendall's tau-b)
```{r}
library(dplyr)
...
```
 ...
 g. for employees who are low (engagement factor composite score < or = 2) on employee_engagement (correlation calculated by conducting kendall's tau-b) = 47 employees only
```{r}
library(dplyr)
